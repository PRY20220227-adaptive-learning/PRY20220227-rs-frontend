function ResourceModal({ isOpen, onClose, resources }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-8 rounded-lg relative opacity-100 transition duration-300">
        ...
        {resources.length === 0 ? (
          <div>
            <div className="text-center text-gray-500 m-6 text-3xl font-semibold">No hay recursos recomendados <span className="mr-2 text-3xl">&#x1F641;</span></div>
          </div>
        ) : (
          resources.map((resource, index) => (
            <div key={index} className="my-4">
              <div className="flex flex-row justify-between">
                ...
                <h1 className="text-left font-medium">{moment.utc(resource.date).format('DD/MM/YYYY')}</h1>
              </div>
